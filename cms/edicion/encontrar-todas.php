<?php 
			...
			for($b = 0; $b <count($contenidos_id); $b++ ){
				...
				if(mysql_num_rows($r_contenidos2) > 0){
					...
					while($contenido2 = mysql_fetch_array($r_contenidos2)){
						...
						array_push($general_arr, array($tabla, $contenido2["id"], $contenido2["titulo"]));
	...
	function finalRuta3 (){
		...
		echo '<br><br>';
		echo 'ARRAY GENERAL CONTENIDOS: <br>';
		echo 'cantidad = ' . count($general_arr) . '<br><br>';
		
		for($h = 0; $h <count($general_arr); $h++ ){
			
			echo $general_arr[$h][0] . ' - ' . $general_arr[$h][1]  . '<br>';
